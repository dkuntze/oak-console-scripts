...
@CompileStatic
class RepoStats {
    final boolean excludeIndexStats = true //Boolean.getBoolean("excludeIndexStats")
	...
	BinaryStats damOriginalBinaryStats = new BinaryStats()
	BinaryStats damBinaryStats = new BinaryStats()
    ...
    int collectAssetStats(NodeTypeStats stats, Tree tree, FluentIterable<Tree> itr) {
        ...
        itr = itr.filter({Tree t ->
            //if (t.name == 'jcr:content' && t.getParent().name == 'original') {
    ...
    def collectBinaryStats(Tree tree) {
        ...
        if (stats) {
        	...
        	if(((SimpleTree)tree).isOriginalPath()){
            	mimeTypeStats[stats.name].add(stats)
            	damOriginalBinaryStats.add(tree, stats.externalSize, stats.count, stats.blobIds, stats.inlinedSize)
            }
            
            if(((SimpleTree)tree).isDamPath()){
            	damBinaryStats.add(tree, stats.externalSize, stats.count, stats.blobIds, stats.inlinedSize)
            }
    ...
    MimeTypeStats getBinaryStats(Tree tree){
        ...
        //if ((size || inlinedSize) && tree.getParent().name=="original"){
        if ((size || inlinedSize)){
    ...
    private void writeStatsInText(nodeTypeStats) {
        statsFileTxt.withPrintWriter { pw ->
            ...
            pw.println("\tAsset Original Renditions            : ${damOriginalBinaryStats.all}")
            pw.println("\tAsset All Renditions          	     : ${damBinaryStats.all}")
            pw.println("\tVersions                             : ${binaryStats.versionedFiles}")
            pw.println()
            pw.println()
    ...
    private void writeBinaryStats(PrintWriter pw) {
        ...
        stats.sort()
        stats = stats.reverse()
        pw.println(dumpStats("Asset Original Rendition stats", stats, columns))
    }
...
<% } %>"""
    ...
    private static class SimpleTree implements Tree{
		...
		boolean isOriginalPath() {
			return path.startsWith("/content/dam") && !path.contains("subassets") && this.getParent().name=="original";
		}
